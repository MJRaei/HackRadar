...
import { listCriteriaSets, deleteCriteriaSet } from '@/lib/api/criteria'
...
export default function CriteriaPage() {
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      await Promise.all(ids.map((id) => deleteCriteriaSet(id)))
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.criteria.all })
      toast.success(`Deleted ${selectedIds.size} criteria set${selectedIds.size !== 1 ? 's' : ''}`)
  ...
  return (
    <div>
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selectedIds.size} criteria set${selectedIds.size !== 1 ? 's' : ''}`}
        message={`Are you sure you want to delete ${selectedIds.size} criteria set${selectedIds.size !== 1 ? 's' : ''}? This cannot be undone.`}
